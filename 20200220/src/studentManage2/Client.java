...
public class Client {
	...
	public void addStudent() {
		...
		for (String string : split) {
			...
			//课程中添加学生id
			Course queryCourseByCId = queryCourseByCId(Integer.parseInt(string));
			queryCourseByCId.getsId().add(student.getsId());
		}
		// 将学生信息添加到学校的学生集合中
	...
	// 根据课程id查询课程信息的方法
		public Course queryCourseByCId(Integer coId) {
			List<Course> cos = School.courses;
			for(int i=0;i<cos.size();i++) {
				if(cos.get(i).getcId() == coId) {
					return cos.get(i);
				}
			}
			return null;
...
//	 查询学生选课情况
	public void queryStudentChooseCourse() {
		...
		System.out.println("id为"+sId+"的学生选择了"+getcId+"课程");
		launchFrame();
	}

//	 查询课程被选情况
	public void queryCourseChooseStudent() {
		System.out.println("所有课程信息:");
		for(Course c:School.courses) {
			System.out.print("课程ID:"+c.getcId()+"课程名字:"+c.getCourseName());
			System.out.println();
		}
		...
		Course queryCourseByCId = queryCourseByCId(cId);
		List<Integer> getsId=queryCourseByCId.getsId();
		
		System.out.println("id为"+cId+"的课程被"+getsId+"学生所选");
		launchFrame();
	}
